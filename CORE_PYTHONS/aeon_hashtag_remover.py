...
def run_test(filepath, online_code):
    ...
    try:
        ...
        subprocess.run(["python", temp_filepath], check=True, capture_output=True, text=True, timeout=30)
        ...
        # 3b. IF IT TIMES OUT, IT'S A SUCCESS! (It means the code is healthy but waiting for input/looping)
        print(f"  SUCCESS (Timeout)! {filename} is healthy and running continuously.")
        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(online_code)
...
def scan_directory():
    ...
    for filename in os.listdir(SOURCE_DIR):
        if filename.endswith(".py"):
            ...
            # SKIP the script itself so it doesn't test itself!
